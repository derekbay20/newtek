jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					slainAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				slainGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			slainGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function slainAppendRow($element, defaultValue ) {
	...
	function slainGetAllInputs($element) {
	 ...
	 */
	 ...
	 $('.multi-image-checkbox').on('change', function () {
	 	slainGetAllImageCheckboxes($(this).parent().parent());
	 });

	// Get the values from the checkboxes and add to our hidden field
	function slainGetAllImageCheckboxes($element) {
	 ...
	 */
	 ...
	 $( ".pill_checkbox_control .sortable" ).sortable({
	 	...
	 	update: function( event, ui ) {
	 		slainGetAllPillCheckboxes($(this).parent());
	 	}
	 });

	 $('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
	 	slainGetAllPillCheckboxes($(this).parent().parent().parent());
	 });

	// Get the values from the checkboxes and add to our hidden field
	function slainGetAllPillCheckboxes($element) {
	 ...
	 */
	 ...
	 $('.google-fonts-list').on('change', function() {
	 	...
	 	var googleFontList = $(this);
	 	var googleFontWrapper = $(this).closest('.google_fonts_select_control');
	 	var elementRegularWeight = googleFontWrapper.find('.google-fonts-regularweight-style');
	 	var elementItalicWeight = googleFontWrapper.find('.google-fonts-italicweight-style');
	 	var elementBoldWeight = googleFontWrapper.find('.google-fonts-boldweight-style');
	 	var selectedFont = googleFontList.val();
	 	var customizerControlName = googleFontList.attr('control-name');
		...
		var indexes = $.map(bodyfontcontrol.slainFontList, function(obj, index) {
		...
		$.each(bodyfontcontrol.slainFontList[index].variants, function(val, text) {
		...
		googleFontWrapper.find('.google-fonts-category').val(bodyfontcontrol.slainFontList[index].category);

		slainGetAllSelects(googleFontWrapper);
	 ...
	 function slainGetAllSelects($element) {
	 	var selectedFont = {
	 ...
	 */
	 ...
	 $('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].slaintinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].slaintinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].slainmediabuttons;
 ...
 */
 ( function( $, api ) {
 	api.sectionConstructor['slain-upsell'] = api.Section.extend( {
